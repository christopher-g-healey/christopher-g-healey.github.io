  ...
  //  dash:     setLineDash pattern, default=solid
{
  ...
  if ( dash.length != 0 ) {		        // Non-default line dash pattern?
    try {
      ...
      console.log( "add_line(), invalid setLineDash option", dash );
    }
  }
...
{
  var  beg;                           // Beg of "<br>\ +<br>" at end of code string
  var  beg_ident;                     // Non-indented code line's width in spaces
  var  code = "";                     // Code from code block
  var  cur_ident;                     // Current indent width
  var  end;                           // End of "<br>\ +<br>" at end of code string
  var  i;                             // Loop counter
  ...
  var  tab_w = 0;                     // Tab width, in spaces
  ...
  // Search for first code line, a line starting with %
  ...
  if ( i == line.length ) {           // No code lines in text provided?
    return;
  }

  // We assume first code line is "%[\ +]..." so indent is however many spaces
  // there are between the % and the first non-space character

  beg_ident = line[ i ].substring( 1 ).search( /[^ ]/ );
  cur_ident = 0;

  // Walk thru tokens on (remaining) lines, as indent size changes,
  // assume new indent block
  ...
  for ( ; i < line.length; i++ ) {
    ...
    // Strip leading % and spaces for non-indented code, if all
    // that's left is spaces, ignore line
    
    line[ i ] = line[ i ].substring( 1 + beg_ident );
    if ( line[ i ].trim().length == 0 ) {
      code = code + " ".repeat( tab_n * tab_w ) + "<br>";
      continue;
    }
    
    // Find first non-space character on line to determine indent in spaces
    
    ident_n = line[ i ].search( /[^ ]/ );

    if ( ident_n != cur_ident ) {     // Indent level change?
      if ( tab_w == 0 ) {             // First indent change? Get tab width
      ...
      // Entirely to handle console (paste on CLI), if we're reducing
      // indent level, AND if the previous line was blank, it'll have
      // spaces between the two previous lines' <br>'s, e.g., "<br>
      // <br>"; we need to remove those spaces to force the console to
      // realize it needs to close the previous indent block. To do
      // that we look for <br>\s+<br> and remove the spaces from
      // between the two <br> strings

        if ( /<br>\ +<br>/.test( code ) ) {
          beg = 0;
          end = code.lastIndexOf( "<br>" );
          code = code.substring( beg, end ).trimEnd() + "<br>";
        }
      }
  ...
  // Entirely to handle console, if the code ends in an indent block,
  // need to strip all the spaces and the final <br> so console knows
  // to end the block(s) and exit the code execution
  
  if ( /\ +<br>/.test( code ) ) {
    end = code.length - "<br>".length;
    code = code.substring( 0, end ).trim() + "<br>";
  }

  // Entirely to handle console, when we completely de-indent, need to
  // add a "<br><br>" to simulate hitting Return twice, which tells
  // console we've completed the line at the end of the indent block,
  // then left all indented code blocks
      
  if ( tab_n != 0 && !code.endsWith( "<br><br>" )) {
    code = code + "<br>";
  }
  ...
  // If code ended inside an indent block, need "<br>" to close that block
...
$(document).ready( function() {
  ...
  w.scroll( function() {
    ...
    //  Width of nav bar is window width minus left offset minus a right
    //  offset equal to the left offset; height of nav bar is just its height

    nav_w = w.width() - ( 2 * nav.offset().left );
    nav_h = $("#nav").height();
